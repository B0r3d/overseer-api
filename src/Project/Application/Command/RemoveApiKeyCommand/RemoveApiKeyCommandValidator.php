...
use Overseer\Project\Domain\Validator\ApiKeyExists;
...
class RemoveApiKeyCommandValidator implements CommandValidator
{
    ...
    {
        if (!$command instanceof RemoveApiKeyCommand) {
            throw new \InvalidArgumentException('Command must be an instance of ' . RemoveApiKeyCommand::class . ' got ' . get_class($command));
        }
        ...
        $validationContext = new ValidationContext([
            ...
            new Field($command->getApiKeyId(), 'Invalid API key', [
                new ValidUuid(),
                new ApiKeyExists($project)
            ])
        ]);
