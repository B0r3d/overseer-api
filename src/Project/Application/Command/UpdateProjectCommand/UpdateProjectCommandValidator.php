...
class UpdateProjectCommandValidator implements CommandValidator
{
    ...
    public function validate(Command $command)
    {
        if (!$command instanceof UpdateProjectCommand) {
            throw new \InvalidArgumentException('Command must be an instance of ' . UpdateProjectCommand::class . ' got ' . get_class($command));
        }
        ...
        $validationContext = new ValidationContext([
            new Field($command->getIssuedBy(), 'Unauthorized request', [
                new AuthorizedProjectMember($project, ProjectMemberPermission::UPDATE_PROJECT()),
            ]),
        ]);
