...
class AcceptInvitationCommandValidator implements CommandValidator
{
    ...
    public function validate(Command $command)
    {
        if (!$command instanceof AcceptInvitationCommand) {
            throw new \InvalidArgumentException('Command must be an instance of ' . AcceptInvitationCommand::class . ' got ' . get_class($command));
        }
        ...
        $validationContext = new ValidationContext([
            ...
            new Field($command->getAcceptedBy(), 'This invitation does not belong to you', [
